// src/pages/EnergyResetOffer.tsx
import { useEffect, useRef } from "react";
import offerImg from "@/assets/coach.png"; // <- replace with your artwork
import { useFacebookPixel } from "@/hooks/useFacebookPixelHome";

export default function EnergyResetOffer() {
  useFacebookPixel();
  const buttonMountRef = useRef<HTMLDivElement | null>(null);

  // Mount Razorpay hosted button (works in React)
  useEffect(() => {
    if (!buttonMountRef.current) return;

    // ensure clean mount on hot reload
    buttonMountRef.current.innerHTML = "";
    const form = document.createElement("form");
    const script = document.createElement("script");
    script.src = "https://checkout.razorpay.com/v1/payment-button.js";
    script.async = true;
    script.setAttribute("data-payment_button_id", "pl_RLQaaaLbrnhTm3"); // <-- your button id
    form.appendChild(script);
    buttonMountRef.current.appendChild(form);
  }, []);
